...
- Экспозиция автоматизаций хаба: локальные параметры `State` каждой включённой автоматизации публикуются как отдельные MQTT/discovery-сущности (выключенные пропускаются, включение/выключение отражается в реальном времени)
