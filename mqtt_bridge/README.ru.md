...
- Экспозиция автоматизаций хаба: каждая автоматизация публикуется с переключателем `enabled` (вкл/выкл) и своими локальными параметрами `State` как MQTT/discovery-сущности (выключенные и без локальных переменных тоже включены)
